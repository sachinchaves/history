  ...
  <div>
    ...
    <ul>
      {album.items.map((item) => (
        <>
        <li key={item.filename}>{item.city} <img src={item.thumbPath} alt={item.caption} /> {item?.geo?.lat}, {item?.geo?.lon}<Link href={`/view/nearby?lat=${item?.geo?.lat}&lon=${item?.geo?.lon}`}>See Nearby</Link></li>
        </>
      ))}
    </ul>
  </div>
